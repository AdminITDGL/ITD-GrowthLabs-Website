...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $video_type   = isset($_POST['video_type'])   ? htmlspecialchars($_POST['video_type'])                : '';
    $message      = isset($_POST['message'])      ? htmlspecialchars($_POST['message'])                   : '';

    // STOPGAP: outbound SMTP is blocked from this droplet (see includes/lead_log.php)
    itdgl_log_lead('videoProductionForm', compact('full_name', 'company_name', 'email', 'phone', 'video_type', 'message'));
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "New Video Production Enquiry: {$full_name}" . ($company_name ? " ({$company_name})" : '') . ($phone ? " — {$phone}" : '');
        $mail->Body    = itdgl_internal_email(
            'Video Production Enquiry',
            ['Full Name' => $full_name, 'Company' => $company_name, 'Email' => $email, 'Phone' => $phone, 'Type of Video' => $video_type, 'Requirements / Message' => $message],
            $phone
        );
        ...
        if ($mail->send()) {
            ...
            $mail->Subject = 'Your Video Production Request is Logged — ITD GrowthLabs';
            $mail->Body    = itdgl_user_email($full_name, ['form_type' => 'video_production']);
            $mail->send();
        }
    } catch (Exception $e) {
        itdgl_log_mail_failure('videoProductionForm', $e->getMessage(), ['name' => $full_name, 'email' => $email]);
    }
} else {
