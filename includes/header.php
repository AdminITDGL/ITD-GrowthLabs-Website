<header>
    <!-- Start Navigation -->
    <nav class="navbar mobile-sidenav navbar-default validnavs dark">

        <!-- Start Top Search -->
        <div class="top-search">
            <div class="container">
                <form action="#">
                    <div class="input-group">
                        <span class="input-group-addon"><i class="fa fa-search"></i></span>
                        <input type="text" class="form-control" placeholder="Search">
                        <span class="input-group-addon close-search"><i class="fa fa-times"></i></span>
                    </div>
                </form>
            </div>
        </div>
        <!-- End Top Search -->

        <div class="container d-flex justify-content-between align-items-center">            
            <!-- Start Header Navigation -->
            <div class="navbar-header">
                <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navbar-menu">
                    <i class="fa fa-bars"></i>
                </button>
                <a class="navbar-brand" href="index.php">
                    <img src="assets/img/logo.png" class="logo" alt="Logo">
                </a>
            </div>
            <!-- End Header Navigation -->

            <!-- Collect the nav links, forms, and other content for toggling -->
            <div class="collapse navbar-collapse" id="navbar-menu">

                <img src="assets/img/logo.png" alt="Logo">
                <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navbar-menu">
                    <i class="fa fa-times"></i>
                </button>

                <ul class="nav navbar-nav navbar-center" data-in="fadeInDown" data-out="fadeOutUp">
                    <li class="dropdown">
                      
                       <li class="dropdown megamenu-fw">
						<a href="#" class="dropdown-toggle" data-toggle="dropdown">Services</a>
						<ul class="dropdown-menu megamenu-content" role="menu">
					<li>
						<div class="row">
							<!-- First Column: Web Development and Content Writing Services -->
							<div class="col-menu col-lg-3">
								<h6 class="title">Website Development</h6>
								<div class="content">
									<ul class="menu-col">
										<li><a href="coming-soon.php">Website Designing</a></li>
										 <li><a href="coming-soon.php">Website Development</a></li>
										<li><a href="coming-soon.php">Website Content Writing</a></li>
									</ul>
								</div>
							</div>

							<!-- Second Column: SEO & Article Writing -->
							<div class="col-menu col-lg-3">
								<h6 class="title">SEO & Content Writing</h6>
								<div class="content">
									<ul class="menu-col">
										<li><a href="coming-soon.php">SEO Article Writing</a></li>
										<li><a href="coming-soon.php">Blog Writing</a></li>
										<li><a href="coming-soon.php">Graphic Design</a></li>
									</ul>
								</div>
							</div>

							<!-- Third Column: Marketing Services -->
							<div class="col-menu col-lg-3">
								<h6 class="title">Marketing Services</h6>
								<div class="content">
									<ul class="menu-col">
										<li><a href="coming-soon.php">Social Media Marketing</a></li>
										<li><a href="coming-soon.php">Search Ads</a></li>
										<li><a href="coming-soon.php">Content Strategy</a></li>
									</ul>
								</div>
							</div>

							<!-- Fourth Column: Case Study & Content Strategy -->
							<div class="col-menu col-lg-3">
								<h6 class="title">Case Studies & Strategy</h6>
								<div class="content">
									<ul class="menu-col">
										<li><a href="coming-soon.php"">Case Study Writing</a></li>
										<li><a href="coming-soon.php">Brand Strategy</a></li>
										<li><a href="coming-soon.php">Content Marketing</a></li>
									</ul>
								</div>
							</div>
						</div>
					</li>
				</ul>
			</li>

                    </li>
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">Resources</a>
                        <!--ul class="dropdown-menu">
                            <li><a href="blog-standard.php">Blog Standard</a></li>
                            <li><a href="blog-with-sidebar.php">Blog With Sidebar</a></li>
                            <li><a href="blog-2-colum.php">Blog Grid Two Colum</a></li>
                            <li><a href="blog-3-colum.php">Blog Grid Three Colum</a></li>
                            <li><a href="blog-single.php">Blog Single</a></li>
                            <li><a href="blog-single-with-sidebar.php">Blog Single With Sidebar</a></li>
                        </ul-->
                    </li>
                    <li><a href="contact-us.php">Contact</a></li>
                </ul>
            </div><!-- /.navbar-collapse -->

            <div class="attr-right">
                <!-- Start Atribute Navigation -->
                <div class="attr-nav flex">
                    <ul>
                        <li class="search"><a href="#"><i class="fas fa-search"></i></a></li>
                        <li class="side-menu">
                            <a href="#">
                                <span class="bar-1"></span>
                                <span class="bar-2"></span>
                                <span class="bar-3"></span>
                            </a>
                        </li>
                    </ul>
                </div>
                <!-- End Atribute Navigation -->

                <!-- Start Side Menu -->
                <div class="side">
                    <a href="#" class="close-side"><i class="icon_close"></i></a>
                    <div class="widget">
						<div class="logo" style="font-size: 30px; font-weight: bold; color: #fff;">
							GrowthLabs
						</div>
					</div>

                    <div class="widget">
                        <p>
                            We are a team of experts providing customized digital marketing solutions, from content writing to content marketing strategies that drive sales.
                        </p>
                    </div>
                    <div class="widget address">
                        <div>
                            <ul>
                                <li>
                                    <div class="content">
                                        <p>Email</p>
                                        <strong>info@itdgrowthlabs.com</strong>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="widget newsletter">
                        <h4 class="title">Get Subscribed!</h4>
                        <form action="#">
                            <div class="input-group stylish-input-group">
                                <input type="email" placeholder="Enter your e-mail" class="form-control" name="email">
                                <span class="input-group-addon">
                                    <button type="submit">
                                        <i class="arrow_right"></i>
                                    </button>  
                                </span>
                            </div>
                        </form>
                    </div>
                    <div class="widget social">
                        <ul class="link">
                            <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                            <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                            <li><a href="#"><i class="fab fa-behance"></i></a></li>
                        </ul>
                    </div>

                </div>
                <!-- End Side Menu -->

            </div>
            <!-- Main Nav -->

        </div>  
        <!-- Overlay screen for menu -->
        <div class="overlay-screen"></div>
        <!-- End Overlay screen for menu --> 
    </nav>
    <!-- End Navigation -->
</header>